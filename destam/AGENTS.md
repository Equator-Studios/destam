...
- **`.shallow()` on an observable root never fires.** `arr.observer` / `obj.observer` are immutable handles to the observable. Their own value (the observable reference) never changes. `.shallow(0)` excludes nested events, so anchoring `.shallow()` at the root captures nothing. Use `.skip().shallow()` instead — `.skip()` advances past the immutable root into the actual contents. Canonical case: `arr.observer.skip().shallow().map(() => arr.length)`. See "Reacting to structural changes" under Common Patterns. This isn't an oversight to patch — no JS mechanism can hook a plain variable rebinding (`let x = a; x = b;` is unobservable, full stop), and a bare `.observer` is exactly such a binding. If you need reactivity to "which object is currently referenced," hold it in `Observer.mutable(...)` and use `.memo()` (or `.unwrap()`) instead of a bare variable — see `docs/observables.md`. Do not "fix" the silent zero-events behavior with a runtime warning (e.g. "this shallow-watch never fired, did you forget `.skip()`?") — it's runtime-indistinguishable from a plain, non-repointable observer used correctly, which is the common case. A generic consumer written once against "some Observer" (`observer.skip().shallow().watch(cb)`) has to behave identically whether handed a plain root or an `Observer.mutable(...).memo()`-wrapped one; that transparency is the point, and a "never fired" diagnostic would false-positive on every correct use of the plain case.
